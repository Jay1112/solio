...
function PlatformItem({ 
    ...
    return (
        <p className='my-4 p-4 rounded-md border-2 border-primary hover:bg-transparent bg-primary cursor-pointer transition-all duration-300 flex items-center justify-start' onClick={handleItemClicked}>
    ...
    return (
        <div>
            ...
            <div className='flex relative'>
                <div className='primary-button cursor-pointer p-2.5 flex items-center justify-center rounded-sm' onClick={(e)=>setShowOptions(!showOptions)}>
                    <i className={clsx(`pi pi-${platform.value} text-xl mr-4`,)}></i>
                    <i className={clsx('pi pi-angle-up text-xl',showOptions && 'rotate-180')}></i>
                </div>
                <p className=' flex-1 flex items-center ml-4 px-2 rounded-sm text-xl z-10'>{platform.label}</p>
                {
                    showOptions &&
